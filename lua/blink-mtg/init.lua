...
function source.new(opts)
	vim.validate("blink-mtg.opts.some_option", opts.some_option, { "string" })
	vim.validate("blink-mtg.opts.optional_option", opts.optional_option, { "string" }, true)
	cards_path = vim.fn.stdpath("data") .. "/mtg-cards.txt"
	print(cards_path)
	cards_file = io.open(cards_path)
	local self = setmetatable({}, { __index = source })
	self.opts = opts
	self.cards = {}

	if cards_file then
		for card in cards_file:lines() do
